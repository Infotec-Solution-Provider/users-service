...
class UserSessions {
	constructor(public readonly data: SessionData, public isPaused: boolean, public onlineTokens: string[] = []) {}
...
class OnlineSessionsService {
	...
	public async addTokenToSession(token: string) {
		...
		let userSessions = sessions.find((s) => s.data.userId === sessionData.userId);
